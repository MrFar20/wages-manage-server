...
### 程序jar包
APP_JAR_NAME="$DIR/cbadmin-app.jar"

### 程序名 无影响
APP_NAME="cbadmin-app"

### 程序类全名
APP_MAIN_CLASS="com.cbadmin.app.CbadminApplication"
...
JAVA_OPTS="java -XX:+UseConcMarkSweepGC -Xmx${Xmx} -Xms${Xms} -Xss${Xss} -XX:+PrintGCDateStamps -XX:+PrintGCDetails  -Xloggc:${LOG_DIR}/${APP_NAME}-gc.log  -XX:-HeapDumpOnOutOfMemoryError -XX:HeapDumpPath=${LOG_DIR}/ -Dfile.encoding=UTF-8 -Dsun.jnu.encoding=UTF-8 -Dspring.config.location=config/ -Dspring.profiles.active=${CONF_ACTIVE}"
...
start() {
  ...
  ###启动
  nohup $JAVA_OPTS -classpath $jarlib ${APP_MAIN_CLASS} > /dev/null 2>&1 &
  ##检查是否启动
  check
  if [ $pid -ne 0 ]; then
...
'update')
  ...
  ;;
*)
  echo -e "使用方法: ./service.sh {start|stop|restart|status|log}\n"
  printf "%-10s->  %s\n" "start" "启用应用"
  printf "%-10s->  %s\n" "stop" "停止应用"
  printf "%-10s->  %s\n" "restart" "重启应用"
  printf "%-10s->  %s\n" "status" "应用状态"
